...
class WGridBase(Editor):
    ...
    def handle_cursor_keys(self, key):
        if not self.total_lines:
            return
        rows_count = self.height - self.y_top_offset - self.y_bottom_offset
        if key == KEY_DOWN:
            if self.cur_line + 1 != self.total_lines:
                ...
                if self.cur_line >= self.top_line + self.height - self.y_top_offset - self.y_bottom_offset:  # cursor went beyond visible area
                    self.top_line += 1
                    self.redraw_lines(self.top_line, rows_count)
            ...
            if self.cur_line + 1 != self.total_lines:  # if not on the very last line
                remains = self.total_lines - (self.top_line + rows_count)
                if 0 < remains:
                    delta = min(remains, rows_count)
                    self.top_line += delta
            ...
            if self.cur_line > 0:  # if not on the very first line
                delta = min(self.top_line, rows_count)
                self.top_line -= delta
