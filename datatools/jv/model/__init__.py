...
def build_root_model(v, k: Optional[Hashable] = None, last_in_parent=True) -> JValueElement:
    model = build_model(v, k, last_in_parent)
    set_indent_recursive(model, 0)
    return model


def build_model(v, k: Optional[Hashable] = None, last_in_parent=True, parent: Optional[JElement] = None) -> JValueElement:
...
def build_model_raw(v, k: Optional[str], last_in_parent=True) -> JValueElement:
    if v is None:
        ...
        return JObject(v, k, build_object_fields_models(v), last_in_parent)
    elif type(v) is list:
        return JArray(v, k, build_array_items_models(v), last_in_parent)
    else:
        v.set_key(k)
        return v


def build_object_fields_models(v) -> List[JValueElement]:
    max_field_name_length = 0 if len(v) == 0 else max(len(k) for k in v)
    fields = []
    i = 0
    size = len(v)
    for k, v in v.items():
        model = build_model(v, k, i >= size - 1)
...
def build_array_items_models(v) -> List[JValueElement]:
    items = []
    size = len(v)
    for i, v in enumerate(v):
        items.append(build_model(v, i, i >= size - 1))
        i += 1
    return items
